...
namespace KerboKatz
{
    ...
    {
        void Log(params object[] debugStrings);
        void Log(LogMode mode, params object[] debugStrings);
    }
    public class KerboKatzBase<T> : MonoBehaviour, ILogHandler
        where T : SettingsBase<T>, new()
    {
        ...
        private void AssetLoaded(GameObject requestedAsset)
        {
            ...
            var uiWindow = GetUIData(requestedAsset.name);
            if (uiWindow != null)
            {
                if (uiWindow.gameObject != null)
                {
